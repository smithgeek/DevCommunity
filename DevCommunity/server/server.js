...
var randomTweetsDb = new db.NeDb('Data/random_tweets.db.json');
...
var meetingIdeasDb = new db.NeDb('Data/meeting_ideas.db.json');
var userVerificationDb = new db.NeDb('Data/user_verification.db.json');
var storyDb = new db.NeDb('Data/stories.db.json');
var userSettingsDb = new db.NeDb('Data/user_settings.db.json');
...
function sendNewMeetingTopicEmails(meeting) {
    userSettingsDb.find({ Condition: { NewMeetingEmailNotification: true } }, function (err, settings) {
        if (err == null) {
...
function sendNewStoryEmails(story) {
    userSettingsDb.find({ Condition: { NewStoryEmailNotification: true } }, function (err, settings) {
        if (err == null) {
...
app.post('/api/restricted/AddMeeting', function (req, res) {
    ...
    if (meeting._id == "") {
        ...
        meetingIdeasDb.update({ Query: condition, Update: { $set: { description: meeting.description, details: meeting.details, date: meeting.date } }, Options: {} }, function (err, numReplaced) {
            if (err != null)
...
app.get('/api/GetSuggestions', function (req, res) {
    meetingIdeasDb.find({ Condition: { $or: [{ date: { $exists: false } }, { date: null }] }, Sort: { vote_count: -1 } }, function (err, suggestions) {
        if (err == null) {
...
app.get('/api/GetArchivedMeetings', function (req, res) {
    meetingIdeasDb.find({ Condition: { $and: [{ date: { $exists: true } }, { $not: { date: null } }] }, Sort: { date: -1 } }, function (err, suggestions) {
        if (err == null) {
            decodeEmail(req, function (email) {
                suggestions.forEach(function (value, index, array) {
                    anonymizeMeeting(value, email);
...
app.post('/api/restricted/Vote', function (req, res) {
    ...
    meetingIdeasDb.find({ Condition: { _id: req.body._id } }, function (err, meetings) {
        if (err == null) {
...
app.get('/api/GetStories', function (req, res) {
    storyDb.find({ Condition: {}, Sort: { timestamp: -1 } }, function (err, stories) {
        if (err == null) {
            decodeEmail(req, function (email) {
...
app.post('/api/restricted/AddStory', function (req, res) {
    ...
    if (story._id == null || story._id == "") {
        ...
        storyDb.update({ Query: { _id: story._id, submittor: getUserEmail(req) }, Update: { $set: { description: story.description, title: story.title, url: story.url } }, Options: {} }, function (err, numReplaced) {
